...
(function () {
    ...
    function openModal() { if (!modal)
        return; modal.style.display = 'flex'; modal.classList.add('is-open'); modal.setAttribute('aria-hidden', 'false'); }
    function closeModal() { if (!modal)
        return; modal.style.display = 'none'; modal.classList.remove('is-open'); modal.setAttribute('aria-hidden', 'true'); localStorage.setItem('privacyModalSeen', 'true'); }
    if (openBtn)
    ...
    modal?.addEventListener('click', e => { if (e.target === modal)
        closeModal(); });
    document.addEventListener('keydown', e => { if (e.key === 'Escape' && modal?.classList.contains('is-open'))
        closeModal(); });
    if (!localStorage.getItem('privacyModalSeen'))
    ...
    if (track) {
        ...
        function go(n) { const len = count(); if (!len)
            return; idx = (n + len) % len; apply(); }
        ...
        window.appendFooterItems = function appendFooterItems(htmlArray) {
            try {
                if (!Array.isArray(htmlArray) || !htmlArray.length)
                    return;
...
//# sourceMappingURL=footer-slider.js.map
// ========================================
// Privacy Modal + Footer Slider Global Init
// ========================================
(function(){
