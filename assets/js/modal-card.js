...
function computeKararNo(r){
  try{
    ...
    var yearIdx = 24; // Y sütunu (0=A..24=Y)
    var yVal = (typeof r[yearIdx] !== 'undefined' && r[yearIdx] != null) ? String(r[yearIdx]).trim() : '';
    var year = (/^\d{4}$/.test(yVal) ? yVal : String(new Date().getFullYear()));
